  ...
  return (
    <SetupLayout>
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-4">Data Overview</h1>
        ...
        <div className="bg-white shadow rounded-lg p-6 mb-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Collect Data</h2>
              <p className="text-gray-600">Run data collection to gather communication information for all employees.</p>
            </div>
            <button
            ...
            >
              {runDataCollectionMutation.isPending ? 'Collecting...' : 'Run Data Collection'}
            </button>
          </div>
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-center">
                ...
                <span className="text-green-800 font-medium">
                  Data collection complete! Information gathered for {employees.filter(emp => emp.status === 'included').length} employees.
                </span>
              </div>
            </div>
          )}
          ...
          <div className="bg-white shadow rounded-lg mb-8">
            <div className="p-6 border-b border-gray-200">
              ...
              <div className="flex items-center text-sm text-gray-500 mt-1">
                ...
                80 issues found
                <button className="ml-4 text-blue-600 hover:text-blue-500">
                  ⚡ Merge All Emails
                </button>
              </div>
            </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Email Count</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Chat Count</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Meeting Count</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">File Access</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Actions</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {employees.filter(emp => emp.status === 'included').map((employee) => (
                    <tr key={employee.id}>
                        ...
                        <div className="flex items-center">
                          ...
                          <div className="ml-3">
                            ...
                            {issues.some(issue => issue.employeeId === employee.id) && (
                              <div className="text-xs text-yellow-600">⚠ Issue</div>
                            )}
                          </div>
                        </div>
                      </td>
        ...
        {issues.length > 0 && (
          <div className="bg-white shadow rounded-lg mb-8">
            <div className="p-6 border-b border-gray-200">
              ...
              <p className="text-gray-600 mt-1">Review and resolve data issues before proceeding with your analysis</p>
            </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-3 gap-4 mb-6">
                ...
                <div className="text-center p-4 bg-orange-50 rounded-lg">
                  ...
                  <div className="text-sm text-gray-600">Data Source Conflicts</div>
                </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center mb-2">
                          ...
                          <div>
                            <div className="font-medium text-gray-900">{issue.employeeName}</div>
                            <div className="text-sm text-gray-600">{issue.type.replace('_', ' ').toUpperCase()}</div>
                          </div>
                        </div>
        ...
        <div className="bg-white shadow rounded-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="text-center p-4 bg-yellow-50 rounded-lg">
              <div className="text-sm font-medium text-yellow-800 mb-2">Email Aliases</div>
              <div className="text-2xl font-bold text-yellow-600">{issuesByType.email_alias.length}</div>
              <div className="text-xs text-yellow-600">employees with multiple emails</div>
              <button className="mt-2 text-xs text-yellow-700 hover:text-yellow-800">Review</button>
            </div>
            
            <div className="text-center p-4 bg-orange-50 rounded-lg">
              <div className="text-sm font-medium text-orange-800 mb-2">Source Conflicts</div>
              <div className="text-2xl font-bold text-orange-600">{issuesByType.data_conflict.length}</div>
              <div className="text-xs text-orange-600">employees with conflicting data</div>
              <button className="mt-2 text-xs text-orange-700 hover:text-orange-800">Review</button>
            </div>
            
            <div className="text-center p-4 bg-red-50 rounded-lg">
              <div className="text-sm font-medium text-red-800 mb-2">Missing Data</div>
              <div className="text-2xl font-bold text-red-600">{issuesByType.missing_data.length}</div>
              <div className="text-xs text-red-600">employees with incomplete information</div>
              <button className="mt-2 text-xs text-red-700 hover:text-red-800">Review</button>
            </div>
          </div>
        </div>
