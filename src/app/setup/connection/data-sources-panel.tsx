...
export default function DataSourcesPanel({ 
  ...
  // Guard clause to ensure connectedSources is always an array
  const safeSources = Array.isArray(connectedSources) ? connectedSources : [];
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Data Sources ({safeSources.length})</h2>
        ...
        {safeSources.length === 0 ? (
          ...
          <div className="space-y-4">
            {safeSources.map((source) => (
              <ConnectedSourceCard
                key={source.connectionId || source.id}
                source={source}
      ...
      {safeSources.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200">
          <p className="text-sm text-gray-500">
            {safeSources.length} connected source{safeSources.length !== 1 ? 's' : ''}
          </p>
        </div>
      )}
    </div>
